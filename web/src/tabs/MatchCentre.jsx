  ...
  return (
    <>
      <div className="glass p-5 sm:p-6">
        <h2 className="text-xl font-semibold">Pre-match Centre</h2>
        <p className="micro-label mb-1 normal-case">Ratings, head-to-head, form, venue edge and toss advice.</p>
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <Field label="Team A">
              <select className={darkInput} value={t1} onChange={(e) => setT1(e.target.value)}>
                {teams.map((t) => <option key={t}>{t}</option>)}
              </select>
            </Field>
            <Field label="Team B">
              <select className={darkInput} value={t2} onChange={(e) => setT2(e.target.value)}>
                {teams.map((t) => <option key={t}>{t}</option>)}
              </select>
            </Field>
          </div>
...
function Result({ d, forms }) {
  ...
  const pills = (name) => (forms?.[name] || []).map((r) => r.winner === name);
  return (
    <div className="fade-in">
      <div className="glass mt-4 overflow-hidden">
        ...
        <p className="micro-label pb-4 text-center">{d.toss.venue}</p>
      </div>
